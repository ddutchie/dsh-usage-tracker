...
function dimOf(e: UsageEntry, groupBy: string): string {
...
function addInto(t: UsageTotals, e: UsageEntry, excludeEstimated: boolean): void {
...
export function filterEntries(entries: UsageEntry[], filter: UsageFilter = {}): UsageEntry[] {
...
export function sumTotals(entries: UsageEntry[], excludeEstimated = false): UsageTotals {
  const t = zeroTotals();
 ...
 */
export function queryUsageOverview(entries: UsageEntry[], filter: UsageFilter = {}): UsageOverview {
  ...
  const dimMap = new Map<string, UsageDimensionBucket>();
  for (const e of inWindow) {
    const dk = dayKey(e.at);
    let d = dayMap.get(dk);
    if (!d) { d = { day: dk, ...zeroTotals() }; dayMap.set(dk, d); }
    addInto(d, e, excludeEstimated);

    const mk = e.model || "unknown";
    let m = modelMap.get(mk);
    if (!m) { m = { model: mk, ...zeroTotals() }; modelMap.set(mk, m); }
    addInto(m, e, excludeEstimated);

    const gk = dimOf(e, groupBy);
    let g = dimMap.get(gk);
    if (!g) { g = { key: gk, ...zeroTotals() }; dimMap.set(gk, g); }
...
export function queryRecent(entries: UsageEntry[], filter: UsageFilter = {}, limit = 100): UsageEntry[] {
