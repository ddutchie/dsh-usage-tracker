...
export function resolvePricing(
  ...
  if (typeof pricing === "function") return pricing(model);
...
export function estimateCostUsd(
  ...
  let cost = 0;
  if (rate.input != null) cost += (uncachedInput / M) * rate.input;
  if (rate.output != null) cost += (completionTokens / M) * rate.output;
  if (rate.cacheRead != null) cost += (cacheReadTokens / M) * rate.cacheRead;
  if (rate.cacheWrite != null) cost += (cacheCreationTokens / M) * rate.cacheWrite;
  return cost;
}
...
export function extractCost(raw: unknown): number | undefined {
  ...
  if (r.cost && typeof r.cost === "object" && typeof r.cost.request_cost_usd === "number") return r.cost.request_cost_usd;
  if (typeof r.request_cost_usd === "number") return r.request_cost_usd;
  if (r.usage && typeof r.usage === "object" && typeof r.usage.cost === "number") return r.usage.cost;
  return undefined;
}
 ...
 */
export function extractCacheTokens(raw: unknown): { cacheReadTokens: number; cacheCreationTokens: number } {
  ...
  else if (typeof u.cache_read_input_tokens === "number") out.cacheReadTokens = u.cache_read_input_tokens;
  else if (typeof u.prompt_cache_hit_tokens === "number") out.cacheReadTokens = u.prompt_cache_hit_tokens;
  else if (typeof u.cachedTokens === "number") out.cacheReadTokens = u.cachedTokens;
  ...
  if (typeof u.cacheCreationTokens === "number") out.cacheCreationTokens = u.cacheCreationTokens;
  else if (typeof u.cacheWriteTokens === "number") out.cacheCreationTokens = u.cacheWriteTokens;
  else if (typeof u.cache_creation_input_tokens === "number") out.cacheCreationTokens = u.cache_creation_input_tokens;
  return out;
}
