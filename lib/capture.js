...
export function applyEvent(s, event) {
    ...
    if (type === "assistant/message") {
        ...
        if (msg?.source?.model)
            s.model = msg.source.model;
        if (msg?.source?.provider)
            s.provider = msg.source.provider;
        const u = msg?.usage ?? data?.usage;
        if (u)
            foldUsage(s, u);
        if (typeof data?.finishReason === "string")
            s.finishReason = data.finishReason;
        else if (typeof msg?.finishReason === "string")
 ...
 */
export function finalizeEntry(s, sessionId, config) {
