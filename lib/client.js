...
// src/client/remote-contribution.ts
function strict(typeSymbol) {
  return { mode: "strict", typeSymbol, schema: external_exports.any() };
}
function method(pkg, ns, name, hasArg) {
  return {
    id: `${pkg}#${ns}/${name}`,
    service: ns,
    namespace: ns,
    ...
    parameters: hasArg ? [{ name: "request", wire: "request", source: "json", codec: strict(`${pkg}#${name}Request`) }] : [],
    result: strict(`${pkg}#${name}Result`)
  };
}
var PACKAGE = "dsh-usage-tracker";
var NS = "usage";
var usageRemoteContribution = {
  package: PACKAGE,
  descriptors: [
...
var UsageRemoteSection = (props) => {
  ...
  import_react2.default.useEffect(() => {
    ...
    (async () => {
      try {
        const ovRes = await props.usage?.overview({ from, to: now, groupBy: "model" });
        const rcRes = await props.usage?.recent({ limit: 12 });
        const ov = ovRes && ovRes.ok ? ovRes.value : ovRes?.totals ? ovRes : void 0;
        const rc = rcRes && rcRes.ok ? rcRes.value ?? [] : Array.isArray(rcRes) ? rcRes : [];
...
function apply(ctx) {
  if (ctx?.slots?.inject) {
    ...
    if (ctx.remote?.$mount) {
      Promise.resolve(ctx.remote.$mount(usageRemoteContribution)).then(() => {
        ctx.inject?.(["slots", "remote.usage"], (c) => {
